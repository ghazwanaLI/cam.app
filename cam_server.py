...
<rect width='100' height='100' rx='20' fill='%231d4ed8'/><text y='75' x='50' text-anchor='middle' font-size='65'>📹</text></svg>">
...
<div id="app" style="display:block">
  ...
  <div id="app-bottom-nav">
    ...
    <div class="bn-wrap">
      ...
      <div class="bn-item" id="bn-field" onclick="bnGoField(this)" style="display:none">
        <div class="bn-ico-wrap">🔧</div>
  ...
  <div id="mob-sidebar-drawer">
    <div style="background:linear-gradient(135deg,#0f172a,#1e3a5f);padding:24px 18px 18px">
      ...
      <div id="drawer-user" style="background:rgba(255,255,255,.08);border-radius:12px;padding:10px 12px;display:flex;align-items:center;gap:10px">
        ...
        <div style="flex:1;min-width:0">
          <div id="drawer-name" style="color:#fff;font-weight:700;font-size:13px">—</div>
          <div id="drawer-role" style="color:rgba(255,255,255,.5);font-size:10px">—</div>
        </div>
      </div>
    </div>
    <div style="padding:12px 8px">
      ...
      <div class="drw-item" id="drw-field" onclick="bnGoField();closeMobDrawer()" style="display:none"><span>🔧</span>الصيانة</div>
      ...
      <div class="drw-item" id="drw-handover" onclick="bnGo('handover','الاستلام والتسليم');closeMobDrawer()" style="display:none"><span>🔄</span>الاستلام والتسليم</div>
      <div class="drw-item" id="drw-users" onclick="bnGo('users','المستخدمون');closeMobDrawer()" style="display:none"><span>🔐</span>المستخدمون</div>
      <div class="drw-item" id="drw-logs" onclick="bnGo('logs','السجل');closeMobDrawer()" style="display:none"><span>📋</span>السجل</div>
    ...
    <div class="topbar-nav">
      ...
      <div class="nav-dropdown">
        ...
        <div class="nav-sub">
          ...
          <div class="nav-sub-item" onclick="_navInv('inv-devices')">🎥 أجهزة التسجيل</div>
          <div class="nav-sub-item" onclick="_navInv('inv-report')">📄 التقارير</div>
        </div>
      </div>
    ...
    <div class="content"><div id="tabs-wrap">
      ...
      <div id="tab-dashboard">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:14px;margin-bottom:18px">
          ...
          <div id="dash-inv-card" onclick="_navInv('inv-stations')" style="background:#fff;border-radius:16px;padding:18px 14px;cursor:pointer;text-align:center;box-shadow:0 4px 16px rgba(16,185,129,.1);border:1.5px solid rgba(16,185,129,.15);transition:all .2s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform='none'">
            <div style="position:relative;width:90px;height:90px;margin:0 auto 10px">
              <svg width="90" height="90" style="transform:rotate(-90deg)">
                <circle cx="45" cy="45" r="38" fill="none" stroke="#d1fae5" stroke-width="8"/>
                <circle id="circ-inv" cx="45" cy="45" r="38" fill="none" stroke="#10b981" stroke-width="8" stroke-linecap="round" stroke-dasharray="239" stroke-dashoffset="239" style="transition:stroke-dashoffset 1s ease"/>
              </svg>
              <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center">
                <div style="font-size:22px">📋</div>
              </div>
            </div>
            <div style="font-size:26px;font-weight:900;color:#10b981;line-height:1" id="s-inv-count">—</div>
            <div style="font-size:12px;font-weight:800;color:#065f46;margin-top:4px">الجرد</div>
            <div style="font-size:10px;color:#94a3b8;margin-top:2px" id="s-inv-sub">محطة مسجلة</div>
          </div>
      ...
      <div id="tab-inventory" style="display:none">
        ...
        <div style="display:flex;gap:0;border-bottom:2px solid #e2e8f0;margin-bottom:0;background:#f8fafc">
          <div class="inv-sub-tab active" id="isub-inv-stations" onclick="switchInvSub('inv-stations')" style="padding:10px 18px;font-size:12px;font-weight:700;cursor:pointer;border-bottom:3px solid #1d4ed8;color:#1d4ed8;background:#fff">📋 جرد المحطات</div>
          <div class="inv-sub-tab" id="isub-inv-devices" onclick="switchInvSub('inv-devices')" style="padding:10px 18px;font-size:12px;font-weight:600;cursor:pointer;color:#64748b;border-bottom:3px solid transparent">🎥 أجهزة التسجيل</div>
          <div class="inv-sub-tab" id="isub-inv-report" onclick="switchInvSub('inv-report')" style="padding:10px 18px;font-size:12px;font-weight:600;cursor:pointer;color:#64748b;border-bottom:3px solid transparent">📄 التقارير</div>
        </div>
              ...
              <table id="inv-list-table" style="direction:rtl;width:100%;border-collapse:collapse">
                <thead>
                  <tr style="background:#1e3a8a;color:#fff">
                    ...
                    <th style="padding:8px 12px;border:1px solid #1e40af;font-size:11px;text-align:right">اسم المحطة</th>
...
async function saveInv(){
  ...
  // قراءة صفوف DVR الديناميكية مع مصدر الطاقة
  const dvrRows=[...document.querySelectorAll('#inv-dvr-body tr.dvr-device-row')].map(tr=>{
    const pwTr=tr.nextElementSibling;
    const pw=pwTr?[...pwTr.querySelectorAll('.dvr-pw-cb:checked')].map(c=>c.value).join(' + '):'';
    return {
...
function printInvSingle(id){
  ...
  if(!filledDvrs.length){
    ...
    filledDvrs.forEach(function(r,i){
      // مصدر الطاقة لكل جهاز
      var pwList=['كهرباء وطني','مولد','عاكسة 4KVA','UPS','طاقة شمسية'];
      var pwSavedR=(r.power||'').split('+').map(function(s){return s.trim();}).filter(Boolean);
      var rowPwHtml='';
      if(pwSavedR.length){
        rowPwHtml='<div style="display:flex;flex-wrap:wrap;gap:3px 6px">';
        pwList.forEach(function(p){
          if(pwSavedR.indexOf(p)>=0){
        ...
        rowPwHtml='<span style="color:#999;font-size:9px">—</span>';
      }
      dvrRows+='<tr>'+
        '<td style="text-align:center;border:1px solid #555;font-weight:800;font-size:10px">'+(i+1)+'</td>'+
...
function goTab(name, el){
  ...
  if(name==='field')      { loadTours(); loadMaintenance(); if(CU?.role!=='admin'){setTimeout(()=>switchField('maintenance',document.getElementById('sub-maint')),50);} }
...
async function initApp(){
  if(CU){
    ...
    // زر إضافة جولة — مدير فقط
    const btnAddTour=document.getElementById('btn-add-tour');
    if(btnAddTour) btnAddTour.style.display=showTours&&can('edit')?'inline-block':'none';
...
async function loadCirculars(){
  try{
    ...
    // badge الموبايل
    const bnBadge=document.getElementById('bn-circ-badge');
    if(bnBadge){bnBadge.style.display=unread?'':'none';bnBadge.textContent=unread;}
    ...
    <table width="100%" style="border-collapse:collapse;font-size:14px">
      ...
      <tbody>${allUsers.map(u=>{
        ...
        const permBadges=u.role==='admin'?'<span class="badge b-navy">كل الصلاحيات</span>':
          Object.entries({edit:'تعديل',del:'حذف',files:'ملفات',reports:'تقارير'})
...
async function doUpload(event, key){
  try{
    ...
    // ضغط الصور تلقائياً: أقصى 1280×960 بجودة 82%
    // جودة الرفع: عادي=82% | عالية=95%
    ...
    // بعد التحويل — دائماً JPG (الصور والـ PDF محوّلة)
...
function _loadMaintPreviews(mid, container){
  ...
  Promise.all(allKeys.map(k=>
    api('GET','/api/files/maint_'+mid+'_'+k)
      .then(res=>{ const d=res.file||res; return (d&&d.data)?{k,d}:null; })
      .catch(()=>null)
  )).then(results=>{
...
async function updateActivityTicker(){
  ...
  try{
    ...
    const items = list.slice(0,20).map(n=>{
      const icon = n.action.includes('جولة')?'🚗':
                   n.action.includes('صيانة')?'🔧':
                   n.action.includes('محطة')?'📍':'📋';
      return `${icon} ${n.user}: ${n.action} — ${n.details} [${n.time}]`;
    });
    tickerContent.textContent = items.join('   ◈   ');
...
let _devRowCount = 0;
const DEVICE_TYPES = ['كاميرا','DVR/NVR','هارد ديسك HDD','سويج POE','UPS عاكس','كابل','إمداد طاقة','أخرى'];
...
function addDeviceRow(data={}){
  ...
  const devOpts  = DEVICE_TYPES.map(t=>`<option value="${t}"${data.device===t?' selected':''}>${t}</option>`).join('');
  const faultOpts = FAULT_TYPES.map(t=>`<option value="${t}"${data.fault===t?' selected':''}>${t}</option>`).join('');

  const div = document.createElement('div');
  div.id = `dev-row-${i}`;
  div.style.cssText = 'display:grid;grid-template-columns:1fr 1fr 70px auto;gap:6px;align-items:center;background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:8px 10px;margin-bottom:6px';
  div.innerHTML = `
    <div>
      <div style="font-size:10px;color:var(--t3);margin-bottom:3px;font-weight:700">نوع الجهاز</div>
      <select class="fi dev-type" dir="rtl" style="width:100%;font-size:12px;padding:5px 8px">
        ${devOpts}
      </select>
    </div>
    <div>
      <div style="font-size:10px;color:var(--t3);margin-bottom:3px;font-weight:700">نوع العطل</div>
      <select class="fi dev-fault" dir="rtl" style="width:100%;font-size:12px;padding:5px 8px">
        ${faultOpts}
      </select>
    </div>
    <div>
      <div style="font-size:10px;color:var(--t3);margin-bottom:3px;font-weight:700">الكمية</div>
...
function getDeviceRows(){
  return [...document.querySelectorAll('#devices-list > div')].map(d=>({
    ...
    fault:  d.querySelector('.dev-fault')?.value||'',
...
function addInvDvrRow(data={}){
  ...
  const pwSaved=(data.power||'').split('+').map(s=>s.trim()).filter(Boolean);
  const pwOpts=[
    {v:'كهرباء وطني',l:'⚡ وطني'},
    {v:'مولد',l:'🔋 مولد'},
    {v:'عاكسة 4KVA',l:'🔌 عاكسة'},
    {v:'UPS',l:'UPS'},
    {v:'طاقة شمسية',l:'☀️ شمسية'}
  ].map(p=>`<label style="display:inline-flex;align-items:center;gap:3px;font-size:10px;cursor:pointer;white-space:nowrap">
    <input type="checkbox" class="dvr-pw-cb" value="${p.v}"${pwSaved.includes(p.v)?' checked':''}> ${p.l}
  </label>`).join('');

  const tr = document.createElement('tr');
  tr.className='dvr-device-row';
  tr.innerHTML=`
    ...
    <td style="padding:2px 4px;border:1px solid #dbeafe;text-align:center">
      <button type="button" onclick="this.closest('.dvr-device-row').nextElementSibling.remove();this.closest('.dvr-device-row').remove();_reindexInvDvr()" style="background:#fee2e2;border:none;border-radius:5px;width:22px;height:22px;cursor:pointer;color:#dc2626;font-size:12px;font-weight:900">✕</button>
    </td>`;
  tb.appendChild(tr);

  // صف مصدر الطاقة
  const pwTr = document.createElement('tr');
  pwTr.className='dvr-pw-row';
  pwTr.innerHTML=`<td colspan="8" style="padding:4px 10px;background:#f0f7ff;border:1px solid #dbeafe;border-top:none">
    <span style="font-size:10px;font-weight:700;color:#1d4ed8;margin-left:8px">⚡ مصدر الطاقة:</span>
    ${pwOpts}
  </td>`;
  tb.appendChild(pwTr);
}
function _reindexInvDvr(){
  let idx=1;
  document.querySelectorAll('#inv-dvr-body tr.dvr-device-row').forEach(tr=>{
    const c=tr.cells[0]; if(c)c.textContent=idx++;
  });
}
...
function exportInvExcel(id){
  ...
  loadXLSX().then(()=>{
    ...
    list.forEach(x=>{
      ...
      if(!cams.length&&(+x.cam_count||0)>0) cams=[{type:x.cam_spec||'IP',indoor:+x.cam_indoor||+x.cam_count||0,outdoor:+x.cam_outdoor||0,lens:x.cam_res||'',note:''}];
      const filledCams=cams.filter(r=>(parseInt(r.indoor)||0)>0||(parseInt(r.outdoor)||0)>0||(r.lens&&r.lens!='—')||r.note);
      const camIn=filledCams.reduce((a,r)=>a+(parseInt(r.indoor)||0),0);
      const camOut=filledCams.reduce((a,r)=>a+(parseInt(r.outdoor)||0),0);
      const total=camIn+camOut||+x.cam_count||0;

      // DVR مع مصدر الطاقة لكل صف
      let dvrs=[];
      try{const dr=x.dvr_rows;dvrs=Array.isArray(dr)?dr:(dr?JSON.parse(dr):[]);}catch(e){}
      if(!dvrs.length&&(x.dvr_model||x.dvr)) dvrs=[{model:x.dvr_model||x.dvr||'',conn:x.dvr_spec||'IP شبكي',hdd_size:x.hdd_size||'',hdd_count:x.hdd_count||0,storage:x.storage_days||x.storage||'',note:'',power:x.power_source||''}];
      const filledDvrs=dvrs.filter(r=>r.model||r.hdd_size||r.storage);

      const stName=x.station_name||x.station||'—';
      const district=x.district||'';
      const address='محافظة صلاح الدين'+(district?' — قضاء '+district+' — مركز القضاء':'');
      const dateStr=new Date().toLocaleDateString('ar-IQ',{year:'numeric',month:'long',day:'numeric'});

      const aoa=[]; const merges=[]; let r=0;

      // عنوان
      aoa.push(['استمارة جرد كاميرات المراقبة وأجهزة التسجيل الفيديوي','','','','','','']); merges.push({s:{r,c:0},e:{r,c:6}}); r++;
      aoa.push(['('+stName+')','','','','','','']); merges.push({s:{r,c:0},e:{r,c:6}}); r++;
      aoa.push([]); r++;

      // معلومات
      aoa.push(['اليوم / التاريخ :', dateStr,'','','','','']); merges.push({s:{r,c:1},e:{r,c:6}}); r++;
      aoa.push(['اسم المنفذ / البناية :', stName,'','','','','']); merges.push({s:{r,c:1},e:{r,c:6}}); r++;
      aoa.push(['العنوان الكامل :', address,'','','','','']); merges.push({s:{r,c:1},e:{r,c:6}}); r++;
      aoa.push(['اسم الدائرة :', 'شركة توزيع المنتجات النفطية / فرع صلاح الدين','','','','','']); merges.push({s:{r,c:1},e:{r,c:6}}); r++;
      if(x.phone){ aoa.push(['البريد / الهاتف :', x.phone,'','نوع النشاط :','أمني رقابي','','']); merges.push({s:{r,c:1},e:{r,c:2}}); merges.push({s:{r,c:4},e:{r,c:6}}); r++; }
      if(x.coords){ aoa.push(['الإحداثيات :', x.coords,'','','','','']); merges.push({s:{r,c:1},e:{r,c:6}}); r++; }
      if(x.notes){ aoa.push(['ملاحظات :', x.notes,'','','','','']); merges.push({s:{r,c:1},e:{r,c:6}}); r++; }
      aoa.push([]); r++;

      // جدول الكاميرات
      aoa.push(['📷 جدول الكاميرات','','','','','','']); merges.push({s:{r,c:0},e:{r,c:6}}); r++;
      aoa.push(['التسلسل','نوع الكاميرا','عدد كاميرات الداخلية','عدد كاميرات الخارجية','حجم العدسة','الملاحظات','']); merges.push({s:{r,c:5},e:{r,c:6}}); r++;
      if(!filledCams.length){ aoa.push(['لا توجد بيانات كاميرات','','','','','','']); merges.push({s:{r,c:0},e:{r,c:6}}); r++;
      } else {
        filledCams.forEach((cam,i)=>{ aoa.push([i+1,cam.type||'IP',parseInt(cam.indoor)||0,parseInt(cam.outdoor)||0,(cam.lens&&cam.lens!='—')?cam.lens:'',cam.note||'','']); merges.push({s:{r,c:5},e:{r,c:6}}); r++; });
        aoa.push(['الإجمالي','',camIn,camOut,total+' كاميرا','','']); merges.push({s:{r,c:0},e:{r,c:1}}); merges.push({s:{r,c:4},e:{r,c:6}}); r++;
      }
      aoa.push([]); r++;

      // جدول أجهزة التسجيل
      aoa.push(['🎥 أجهزة التسجيل الفيديوي','','','','','','']); merges.push({s:{r,c:0},e:{r,c:6}}); r++;
      aoa.push(['#','نوع جهاز التسجيل','طريقة الربط','سعة الهارد (TB)','عدد الهاردات','فترة التسجيل','مصدر الطاقة']); r++;
      if(!filledDvrs.length){ aoa.push(['لا توجد بيانات أجهزة تسجيل','','','','','','']); merges.push({s:{r,c:0},e:{r,c:6}}); r++;
      } else {
        filledDvrs.forEach((d,i)=>{
          aoa.push([i+1, d.model||'', d.conn||'', d.hdd_size||'', d.hdd_count||'', d.storage||'', (d.power||'').replace(/\+/g,' + ')]);
          r++;
        });
      }
      aoa.push([]); r++;
      ...
      // التوقيعات
      aoa.push(['مسؤول الشعبة','','','','مدير الدائرة','','']); merges.push({s:{r,c:0},e:{r,c:3}}); merges.push({s:{r,c:4},e:{r,c:6}}); r++;
      aoa.push(['التوقيع : ___________________','','','','التوقيع : ___________________','','']); merges.push({s:{r,c:0},e:{r,c:3}}); merges.push({s:{r,c:4},e:{r,c:6}}); r++;

      const ws = XLSX.utils.aoa_to_sheet(aoa);
      ws['!merges']=merges;
      ws['!cols']=[{wch:20},{wch:22},{wch:16},{wch:14},{wch:16},{wch:22},{wch:22}];
      ws['!dir']='rtl';
      const shName=stName.substring(0,28).replace(/[\\\/\?\*\[\]:]/g,'');
      XLSX.utils.book_append_sheet(wb, ws, shName||('محطة_'+x.id));
    });

    const fname = id
      ? 'جرد_'+(list[0]?.station_name||id)+'_'+new Date().toISOString().split('T')[0]+'.xlsx'
      : 'جرد_الكاميرات_'+new Date().toISOString().split('T')[0]+'.xlsx';
  ...
  }).catch(()=>toast('خطأ في تحميل مكتبة Excel','err'));
}
...
function printHandover(id){
  ...
  w.document.write('<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8">'
    ...
    +'@media print{.np{display:none}@page{margin:8mm}}</style></head><body>'
    +'<div class="np" style="margin-bottom:10px"><button onclick="window.print()" style="background:#1d4ed8;color:#fff;border:none;padding:7px 18px;border-radius:5px;cursor:pointer;font-weight:700">🖨️ طباعة</button></div>'
...
async function _autoRefreshTick(){
  ...
  _arCount++;
  try{
    // كل دورة: تحديث الصيانة
    const dm = await api('GET','/api/maintenance');
    const nm = dm.maintenance||dm.items||[];
    if(nm.length !== allMaint.length || JSON.stringify(nm.slice(0,3)) !== JSON.stringify(allMaint.slice(0,3))){
      allMaint = nm;
      if(typeof renderMaint==='function') renderMaint();
      if(typeof updateDashboard==='function') updateDashboard();
      _showUpdateToast('تم تحديث بيانات الصيانة');
    }
    // كل 3 دورات (3 دقائق): تحديث المحطات والجرد
    if(_arCount % 3 === 0){
      const ds = await api('GET','/api/stations');
      allStations = ds.stations||[];
      if(typeof renderStations==='function') renderStations();
    }
    // حفظ نسخة احتياطية بعد كل تحديث
    _saveBackup();
  }catch(e){
...
function _showUpdateToast(msg){
  ...
  _updateToastTimer = setTimeout(()=>{ el.style.opacity='0'; }, 3000);
}

function startAutoRefresh(){
  if(_arInterval) clearInterval(_arInterval);
  _arInterval = setInterval(_autoRefreshTick, 60000); // كل دقيقة
}
...
<div id="maint-ticker-bar" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:9000;background:linear-gradient(90deg,#1e3a5f,#1d4ed8);color:#fff;padding:7px 14px;font-family:'Cairo',sans-serif;font-size:12px;direction:rtl;box-shadow:0 -2px 14px rgba(0,0,0,.25);align-items:center;gap:10px">
  <span id="ticker-label" style="background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.25);border-radius:20px;padding:2px 10px;font-weight:800;font-size:10px;white-space:nowrap;flex-shrink:0">📡 فعاليات المخولين</span>
  <div style="overflow:hidden;flex:1;position:relative;height:20px">
    <div id="maint-ticker-content" style="white-space:nowrap;position:absolute;animation:tickerScroll 120s linear infinite"></div>
  </div>
...
<style>
@media print{
  body > *:not(#inv-print-overlay){display:none!important}
